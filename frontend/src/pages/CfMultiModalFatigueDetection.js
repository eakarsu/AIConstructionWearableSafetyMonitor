...
/**
 * Gap/CFS Feature Page: Multi-modal fatigue detection
 * Slug: multi-modal-fatigue-detection
 * Backend: /api/cf-multi-modal-fatigue-detection
 */
export default function MultiModalFatigueDetectionPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/cf-multi-modal-fatigue-detection', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>Multi-modal fatigue detection</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>Combine wearable biometrics (HR, movement), computer vision (eye tracking), and ambient sensors for early fatigue alerts.</p>
