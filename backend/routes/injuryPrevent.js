...
const FEATURE_KEY = process.env.FEATURE_INJURY_PREVENT_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Predictive injury prevention.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIConstructionWearableSafetyMonitor - injury-prevent',
    },
    body: JSON.stringify({
...
router.post('/injury-prevent', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_INJURY_PREVENT_KEY');
    const ai = await callLLM({ feature: 'injury-prevent', goal: 'Predictive injury prevention', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['injury-prevent', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[injury-prevent] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'injury-prevent',
      endpoint: '/api/ai/injury-prevent',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[injury-prevent] error:', err.message);
    return res.status(err.statusCode || 500).json({ error: err.message || 'Internal error' });
  }
});

router.get('/injury-prevent/health', (req, res) => {
  res.json({
    feature: 'injury-prevent',
    endpoint: '/api/ai/injury-prevent',
