/**
 * Custom Feature: cohort-analysis
 * High-risk team cohort analysis
 *
 * POST /api/ai/cohort-analysis
 * Auth required. Generated as part of Custom Feature Suggestions scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_COHORT_ANALYSIS_KEY
...
const FEATURE_KEY = process.env.FEATURE_COHORT_ANALYSIS_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: High-risk team cohort analysis.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIConstructionWearableSafetyMonitor - cohort-analysis',
    },
    body: JSON.stringify({
...
router.post('/cohort-analysis', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_COHORT_ANALYSIS_KEY');
    const ai = await callLLM({ feature: 'cohort-analysis', goal: 'High-risk team cohort analysis', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['cohort-analysis', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[cohort-analysis] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'cohort-analysis',
      endpoint: '/api/ai/cohort-analysis',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
...
router.get('/cohort-analysis/health', (req, res) => {
  res.json({
    feature: 'cohort-analysis',
    endpoint: '/api/ai/cohort-analysis',
