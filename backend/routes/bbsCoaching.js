/**
 * Custom Feature: bbs-coaching
 * Behavior-based safety coaching
 *
 * POST /api/ai/bbs-coaching
 * Auth required. Generated as part of Custom Feature Suggestions scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_BBS_COACHING_KEY
...
const FEATURE_KEY = process.env.FEATURE_BBS_COACHING_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Behavior-based safety coaching.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIConstructionWearableSafetyMonitor - bbs-coaching',
    },
    body: JSON.stringify({
...
router.post('/bbs-coaching', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_BBS_COACHING_KEY');
    const ai = await callLLM({ feature: 'bbs-coaching', goal: 'Behavior-based safety coaching', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['bbs-coaching', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[bbs-coaching] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'bbs-coaching',
      endpoint: '/api/ai/bbs-coaching',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[bbs-coaching] error:', err.message);
    return res.status(err.statusCode || 500).json({ error: err.message || 'Internal error' });
  }
});

router.get('/bbs-coaching/health', (req, res) => {
  res.json({
    feature: 'bbs-coaching',
    endpoint: '/api/ai/bbs-coaching',
